...
bool spmv_test_harness (
    ...
    float ALPHA = 1.0, BETA = 0.0, EWISE_VAL = 1.0;

    // initiate vec X and vec Y
    vector<float> vec_X_cpu, vec_Y_out_cpu;
    vec_X_cpu.resize(K, 0.0);
    vec_Y_out_cpu.resize(M, 0.0);
    ...
    cout << "Generating vector Y out for ewise-only ...";
    for (int mm = 0; mm < M; ++mm) {
        vec_Y_out_cpu[mm] = -2.0 * (mm + 1);
    }
    ...
    // Notice!!! use vec_Y_out not vec_Y
    for (int mm = 0; mm < M; ++mm) {
        vec_Y_out_fpga[mm] = vec_Y_out_cpu[mm];
    }
    ...
    inst.SetArg(0, buffer(tmp_i));
    for (int i = 1; i <= 24; ++i) inst.SetArg(i, buffer(tmp_ul));
    inst.SetArg(25, enable(vec_X_fpga));
    inst.SetArg(26, buffer(tmp_f));
    inst.SetArg(27, buffer(tmp_f));
    ...
    printf("Kernel time is %f ms\n", inst.ComputeTimeSeconds()*1000);
    */

    for (int mm = 0; mm < M; ++mm) {
        vec_Y_out_cpu[mm] = vec_Y_out_cpu[mm] + EWISE_VAL;
    }
    bool pass = verify(vec_Y_out_cpu, vec_X_fpga, M);
    if(pass){
        cout << "Success!\n";
    } else{
        cout << "Failed.\n";
