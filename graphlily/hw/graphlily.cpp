            ...
            if (!fifo_A.empty()) {
                ...
                for (int p = 0; p < 8; ++p) {
                    ...
                    if (a_row[17] == 0) {
                        ...
                        raxv.axv[p] = pe_float_mul_alu(a_val_f, local_X[p/2][a_col], semi_op) /*Multiplication*/ ;
                    }
                }
...
#pragma HLS inline
    ...
    float c_val_plus_d0_f = pe_float_add_alu(tapa::bit_cast<float>(c_val_u), val_d0_f, semi_op) /*Addition*/ ;
...
#pragma HLS pipeline II=1
        for (int p = 0; p < 8; ++p) {
            ap_uint<32> zero = tapa::bit_cast<ap_uint<32>>(semi_zero) /*SemiringZero*/;
            local_C[p][i](31, 0) = zero(31, 0);
            local_C[p][i](63, 32) = zero(31, 0);
        }
    }
...
aby:
    for (int i = 0, c_idx = 0, o_idx = 0; i < num_pe_output;) {
#pragma HLS loop_tripcount min=1 max=1800
#pragma HLS pipeline II=1
...
void MaskOut_AX(tapa::istream<float_v16> & fifo_in,
             tapa::istream<float_v16> & fifo_MK,
             tapa::ostream<float_v16> & fifo_out,
             const float semi_zero,
    ...
    for (;;) {
#pragma HLS pipeline II=1
        bool flag_nop = fifo_in.empty() | fifo_MK.empty();
        if (!flag_nop) {
            float_v16 tmp0; fifo_in.try_read(tmp0);
            float_v16 tmp1; fifo_MK.try_read(tmp1);
            float_v16 tmpv16;
#pragma HLS aggregate variable=tmpv16
            for (int k = 0; k < 16; ++k) {
#pragma HLS unroll
                bool do_write = false; // do write after checking mask
                switch (mask_type) {
                    case NOMASK:
                        do_write = true;
                        break;
...
void FloatvMultConst(const int M,
                     const int alpha_u,
                     const char mode,
...
#pragma HLS pipeline II=1
        ...
        if (read_ready) {
            float_v16 c_out = tmp * /*NotSemiringMultiplication*/ alpha_f;
...
void read_MK(const int M,
            const char mode,
            tapa::async_mmap<float_v16> & vec_MK,
            tapa::ostream<float_v16> & fifo_MK
            ) {
    if (~mode & MODE_SPMV) return;

    const int num_ite_MK = (M + 15) >> 4;

    rd_MK:
    for(int i_req = 0, i_resp = 0; i_resp < num_ite_MK;) {
#pragma HLS loop_tripcount min=1 max=500000
#pragma HLS pipeline II=1
        async_read(vec_MK,
                    fifo_MK,
...
#pragma HLS pipeline II=1
        ...
        if (!flag_nop) {
            ...
            float_v16 c_out = tmp0 + tmp1 /*NotSemiringAddition*/ ;
            fifo_out.write(c_out);
        }
    }
}

void write_Y(const int M,
             const char mode,
...
//-------------------------------------------------------------------------
// GraphLily Element-wise Add Dense
//-------------------------------------------------------------------------

void read_Y_out(const int M,
                const char mode,
                tapa::async_mmap<float_v16> & Y_out,
                tapa::ostream<float_v16> & fifo_Y_out_to_X
                ) {
    if (~mode & MODE_EWISE_ADD) return;

    const int num_ite_Y_out = (M + 15) >> 4;

    rd_Y:
    for(int i_req = 0, i_resp = 0; i_resp < num_ite_Y_out;) {
#pragma HLS loop_tripcount min=1 max=500000
#pragma HLS pipeline II=1
        async_read(Y_out,
                    fifo_Y_out_to_X,
...
void EwiseAddDense(const int M,
                   const int add_val_int,
                   const char mode,
                   tapa::istream<float_v16> & fifo_in,
                   tapa::ostream<float_v16> & fifo_out
                   ) {
    if (~mode & MODE_EWISE_ADD) return;
    const float add_val_f = tapa::bit_cast<float>(add_val_int);
    const int num_ite_Y_out = (M + 15) >> 4;
cc:
    for (int i = 0; i < num_ite_Y_out;) {
#pragma HLS pipeline II=1
        float_v16 tmp;
        bool read_ready = fifo_in.try_read(tmp);
        if (read_ready) {
            float_v16 c_out = tmp + add_val_f;
            fifo_out.write(c_out);
            ++i;
        }
    }
...
void write_X(const int M,
             const char mode,
             tapa::istream<float_v16> & fifo_Y_out_to_X,
...
#pragma HLS loop_tripcount min=1 max=500000
#pragma HLS pipeline II=1
        if ((i_req < num_ite_Y_out) &
            !fifo_Y_out_to_X.empty() &
            !X.write_addr.full() &
            !X.write_data.full() ) {
            X.write_addr.try_write(i_req);
            float_v16 tmpv16;
            fifo_Y_out_to_X.try_read(tmpv16);
            X.write_data.try_write(tmpv16);
            ++i_req;
        }
        uint8_t n_resp;
        if (X.write_resp.try_read(n_resp)) {
            i_resp += int(n_resp) + 1;
        }
    }
}
...
//-------------------------------------------------------------------------
// GraphLily Dense Vector Assign Mask
//-------------------------------------------------------------------------

void read_X_as_MK(const int K,
                  const char mode,
                  tapa::async_mmap<float_v16> & X,
                  tapa::ostream<float_v16> & fifo_out
                  ) {
    if (~mode & MODE_DENSE_ASSIGN) return;

    const int num_ite_Y_out = (K + 15) >> 4;
...
void read_MK_as_assign_inout(const int M,
                            const char mode,
                            tapa::async_mmap<float_v16> & MK,
                            tapa::ostream<float_v16> & fifo_out
                            ) {
    if (~mode & MODE_DENSE_ASSIGN) return;
...
void AssignVectorDense(const int M,
                       const int assign_val_int,
                       const char mask_type,
                       const char mode,
                       tapa::istream<float_v16> & fifo_MK,
                       tapa::istream<float_v16> & fifo_assign_in,
                       tapa::ostream<float_v16> & fifo_assign_out
                       ) {
    if (~mode & MODE_DENSE_ASSIGN) return;
    const float assign_val_f = tapa::bit_cast<float>(assign_val_int);
    const int num_ite_Y_out = (M + 15) >> 4;
cc:
    for (int i = 0; i < num_ite_Y_out; ++i) {
#pragma HLS pipeline II=1
        float_v16 tmp_mask = fifo_MK.read();
        float_v16 tmp_inout = fifo_assign_in.read();

        for (int k = 0; k < 16; k++) {
#pragma HLS unroll
            if (mask_type == WRITETOZERO) {
                if (tmp_mask[k] == 0) {
                    tmp_inout[k] = assign_val_f;
                }
        ...
        fifo_assign_out.write(tmp_inout);
    }
}

void MK_as_assign_inout(const int M,
                        const char mode,
                        tapa::istream<float_v16> & fifo_assign_out,
                        tapa::async_mmap<float_v16> & MK
                        ) {
    if (~mode & MODE_DENSE_ASSIGN) return;
...
#pragma HLS pipeline II=1
        if ((i_req < num_ite_Y_out) &
            !fifo_assign_out.empty() &
            !MK.write_addr.full() &
            !MK.write_data.full() ) {
            MK.write_addr.try_write(i_req);
            float_v16 tmpv16;
            fifo_assign_out.try_read(tmpv16);
            MK.write_data.try_write(tmpv16);
            ++i_req;
        }
        uint8_t n_resp;
        if (MK.write_resp.try_read(n_resp)) {
            i_resp += int(n_resp) + 1;
        }
    }
}
...
void Serpens(tapa::mmap<int> edge_list_ptr,
             ...
             const int alpha_u,
             const int beta_u,
             const int ewise_add_val, // TODO: cast
             const int assign_val, // TODO: cast
             const float semi_zero,
             const char semi_op,
             const char mask_type,
             const char mode
    ...
    tapa::stream<float_v16, FIFO_DEPTH> fifo_MK("fifo_MK");

    tapa::stream<float_v16, FIFO_DEPTH> fifo_Y_AX_MK("fifo_Y_AX_MK");

    /* =========element-wise add dense======= */

    tapa::stream<float_v16, FIFO_DEPTH> fifo_Y_out_to_X_in("fifo_Y_out_to_X_in");

    tapa::stream<float_v16, FIFO_DEPTH> fifo_Y_out_to_X_out("fifo_Y_out_to_X_out");

    /* =========assign vector dense======= */

    tapa::stream<float_v16, FIFO_DEPTH> fifo_X_as_MK("fifo_X_as_MK");

    tapa::stream<float_v16, FIFO_DEPTH> fifo_MK_as_assign_inout("fifo_MK_as_assign_inout");

    tapa::stream<float_v16, FIFO_DEPTH> fifo_assign_out_to_MK("fifo_assign_out_to_MK");
    ...
    tapa::task()
        ...
        .invoke<tapa::join>(EwiseAddDense,
                            M,
                            ewise_add_val,
                            mode,
                            fifo_Y_out_to_X_in,
        ...
        .invoke<tapa::join>(read_MK_as_assign_inout,
                            M,
                            mode,
                            vec_MK,
                            fifo_MK_as_assign_inout
                            )

        .invoke<tapa::join>(AssignVectorDense,
                            M,
                            assign_val,
                            mask_type,
                            mode,
                            fifo_X_as_MK,
                            fifo_MK_as_assign_inout,
                            fifo_assign_out_to_MK
                            )

        .invoke<tapa::join>(MK_as_assign_inout,
                            M,
                            mode,
                            fifo_assign_out_to_MK,
                            vec_MK
