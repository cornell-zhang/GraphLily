...
public:
     ...
     */
    graphblas::aligned_index_float_struct_t compute_reference_results(graphblas::aligned_index_float_struct_t &vector);
...
void SpMSpVModule<matrix_data_t, vector_data_t, val_index_t>::generate_kernel_ini() {
    ...
    ini << "sp=kernel_spmspv_v5_1.mat_idxptr_ddr:DDR[1]" << std::endl;
...
template<typename matrix_data_t, typename vector_data_t, typename val_index_t>
graphblas::aligned_index_float_struct_t
SpMSpVModule<matrix_data_t, vector_data_t, val_index_t>::compute_reference_results(graphblas::aligned_index_float_struct_t &vector) {
  // measure dimensions
  unsigned int vec_nnz_total = vector[0].index;
  unsigned int num_rows = this->csc_matrix_.num_rows;

  // create result container
  aligned_index_float_struct_t reference_results;
  ...
  index_float_struct_t reference_head;
...
SpMSpVModule<matrix_data_t, vector_data_t, val_index_t>::compute_reference_results(graphblas::aligned_index_float_struct_t &vector,
  ...
  for (size_t obid = 0; obid < num_rows; obid++) {
    ...
    if(do_write) {
      if(output_buffer[obid]) {
        graphblas::index_float_struct_t a;
        a.val = output_buffer[obid];
        a.index = obid;
        reference_results.push_back(a);
        nnz_cnt ++;
