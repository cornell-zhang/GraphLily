...
int main(int argc, char *argv[]) {
    ...
    std::generate(vector.begin(), vector.end(), [&](){return data_t(std::rand() % 256);});
    ...
    std::vector<data_t, aligned_allocator<data_t>> reference_results(num_rows);
    ...
    /*
    std::string kernel_name_full = kernel_name + ":{" + "kernel_spmv_v3_" + "1" + "}";
    */
    cl_int err;
    cl::CommandQueue q;
